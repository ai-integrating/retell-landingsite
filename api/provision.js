...
function buildSchedulerEmailGateBlock() {
  return [
    ...
    `- Ask: "Would you like me to check availability for you?"`,
    `- Only proceed after the caller agrees.`,

    `STEP 3 — CHECK AVAILABILITY`,
    `- Say: "Let me check the schedule for you."`,
    `- Call the availability tool using the selected service.`,
    `- Do NOT guess or suggest times manually.`,

    `STEP 4 — OFFER FIRST AVAILABLE`,
    `- Offer ONE slot at a time.`,
    `- ALWAYS start with the earliest available slot returned.`,
    `- Example: "The first available I have is Tuesday at 10:00 AM. Would you like that?"`,
    `- If declined, offer the next available slot.`,
    `- Do NOT list multiple times unless asked.`,

    `STEP 5 — SLOT CONFIRMATION (CRITICAL GATE)`,
    `- Wait until the caller clearly accepts a specific time.`,
    `- Do NOT collect personal details before a slot is chosen.`,
    ...
    `  - a slot is accepted`,
    `  - name, email, and phone are collected`,
    `- Use: selected_start, service_key, name, email, phone`,

    `STEP 8 — CONFIRMATION`,
    `- Do NOT say the appointment is booked unless the tool succeeds.`,
    `- If successful: confirm clearly.`,
    `- If failed: apologize and offer next steps.`,
